...
extern const int				ADC_Num_Bits;
extern const int				Num_ADC_Vals;
extern const long				Min_Count;
extern const long				Max_Count;
const float						Version_Number = 2.000;	
...
//Function: init
//Description: Creates pointers to data structures contained by TwoPhotonGui to set up acquisition
void TwoPhotonThread::init(ScanEngine *scanEngi, AcqEngine *acqEngi,  DataFile2P *data2Pi, zStepperDriver *zStepper, AomControl *aCtrl)
{
...
TwoPhotonThread::~TwoPhotonThread()
{
...
void TwoPhotonThread::run()
{
	...
	//aomCtrl->startDAQmxTask();  Move this to after we move in z
		...
		for (iZCount = 0; iZCount < numZSteps; iZCount++)
		{
			...
			if(numZSteps>1)
			{
				...
				if(aomCtrl->getBIntScaling())
				{
					...
					AomUpdate(debug);		//pass relative depth, not zpos
				}
...
int TwoPhotonThread::ProcessDataForDisplay(int linescan)
{
	...
	{
		delete [] imageData2;
		imageData2 = new unsigned char[numValidXSamps * numValidYSamps];
	}
	...
	for (y = 0; y < numValidYSamps; y++)
	{
		for(x = acqEng->getXOffset() + overScan; x < numValidXSamps + acqEng->getXOffset() + overScan; x++)
		{
			...
			if (acqEng->getBInput1())
			{
				...
				{
					imageData2[pxlCount]=(unsigned char)(((tempVal - (double)min1)/((double)max1 - (double)min1)) * 255);
				}
...
ContourInfo2* TwoPhotonThread::getContourVision(int i)
{
	ROI*			curr_roi;
...
void TwoPhotonThread::updateDataPt()
{
...
void TwoPhotonThread::toggleImage2()
{
...
void TwoPhotonThread::setIntensityScaling(short sMax1, short sMin1, short sMax2, short sMin2)
{
	scaleMax1 = sMax1;
	scaleMin1 = sMin1;
	scaleMax2 = sMax2;
...
int TwoPhotonThread::CalcNumZSteps()
{
	int numZSteps;
...
int TwoPhotonThread::Update2PDataStruct()
{
...
double TwoPhotonThread::CalcIntensityScalingCoeff()
{
	double soln;
...
double TwoPhotonThread::CalcAomIntVoltage(double depth)
{
...
void TwoPhotonThread::AomUpdate(double voltage)
{
...
int TwoPhotonThread::CalcXYVoltsFromPxlVal(int x, int y, double& xVolts, double& yVolts)
{
